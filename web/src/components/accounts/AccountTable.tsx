import { ChevronIcon, PowerIcon, RefreshIcon, TrashIcon } from '../Icons'
...
export function AccountTable({ accounts, busyID, resetBusyID, resetStates, onModels, onRefresh, onResetLoad, onReset, onToggle, onRemove }: { accounts: ProviderAccount[]; busyID: string; resetBusyID: string; resetStates: Record<string, ResetCreditState>; onModels: (account: ProviderAccount) => void; onRefresh: (account: ProviderAccount) => void; onResetLoad: (account: ProviderAccount) => void; onReset: (account: ProviderAccount, creditID?: string) => void; onToggle: (account: ProviderAccount) => void; onRemove: (account: ProviderAccount) => void }) {
  return <div className="table-frame account-table-frame"><table className="account-table">
    ...
    <tbody>{accounts.map((account) => {
      ...
      return <tr key={account.id}>
        ...
        <td><div className="row-actions">{account.credential_type !== 'api_key' ? <button className="row-action-button" type="button" aria-label={`Refresh credentials for ${account.display_name}`} title="Refresh credentials" onClick={() => onRefresh(account)} disabled={busy}>{busyID === account.id ? <Spinner /> : <RefreshIcon />}</button> : null}<button className="row-action-button" type="button" aria-label={`${account.status === 'disabled' ? 'Enable' : 'Disable'} ${account.display_name}`} title={account.status === 'disabled' ? 'Enable account' : 'Disable account'} onClick={() => onToggle(account)} disabled={busy}><PowerIcon /></button><button className="row-action-button row-action-button--danger" type="button" aria-label={`Remove ${account.display_name}`} title="Remove account" onClick={() => onRemove(account)} disabled={busy}><TrashIcon /></button></div></td>
      </tr>
    })}</tbody>
  </table></div>
}
