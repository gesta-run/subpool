...
func (s *responsesWSSession) readClient() {
	...
	for {
		...
		bodyTimer := time.AfterFunc(bodyState.ReadTimeout, cancel)
		payload, releaseBody, err := s.hub.backend.ReadRequestBody(reader)
		bodyTimer.Stop()
		cancel()
		if err != nil {
			reason := "read_failed"
		...
		if !s.handleRequest(request, releaseBody) {
			return
		}
	}
}

func (s *responsesWSSession) handleRequest(request responsesWSRequest, releaseBody func()) bool {
	defer func() {
		if releaseBody != nil {
			releaseBody()
		}
	...
	if requestErr = s.hub.backend.AllowAPIKeyRequest(s.ctx, route); requestErr != nil {
	...
	turn, requestErr := s.reserveTurn(request, payload, releaseBody)
	if requestErr != nil {
		s.hub.turnsRejected.Add(1)
		s.sendError(request.streamID(), requestErr.Code, requestErr.Message)
		return true
...
func (s *responsesWSSession) resolveRoute() (domain.KeyRoute, *RequestError) {
	s.mu.Lock()
	pinned := s.pinned
	...
	if pinned {
		route, err = s.hub.backend.RoutingStore().ResolvePinnedAPIKey(s.ctx, s.keyDigest, s.poolID, accountID)
	} else {
		route, err = s.hub.backend.RoutingStore().ResolveAPIKey(s.ctx, s.keyDigest)
	}
	if err != nil {
		if errors.Is(err, store.ErrPinnedUnavailable) {
			return domain.KeyRoute{}, &RequestError{http.StatusServiceUnavailable, "session account is unavailable", "subpool_session_account_unavailable"}
		}
		if errors.Is(err, store.ErrNotFound) {
			return domain.KeyRoute{}, &RequestError{http.StatusUnauthorized, "invalid API key", "invalid_api_key"}
		}
		return domain.KeyRoute{}, &RequestError{http.StatusServiceUnavailable, "API key state is unavailable", "server_error"}
	}
	if !scopeAllowed(route.Key.Scopes, "responses") {
		return domain.KeyRoute{}, &RequestError{http.StatusForbidden, "API key scope does not allow this endpoint", "insufficient_scope"}
	}
	if pinned {
		if !route.MembershipEnabled || !s.hub.backend.AccountHealthy(route.Account) {
			return domain.KeyRoute{}, &RequestError{http.StatusServiceUnavailable, "session account is unavailable", "subpool_session_account_unavailable"}
		}
		s.mu.Lock()
...
func (s *responsesWSSession) ensurePinned(request responsesWSRequest, route domain.KeyRoute) (bool, bool, *RequestError) {
	s.mu.Lock()
	if s.pinned {
		native := s.native
		...
		s.mu.Unlock()
		if switching {
			return false, false, &RequestError{http.StatusServiceUnavailable, "provider account failover is in progress", "provider_error"}
		}
		if native && request.Model != upstreamModel {
			return false, false, &RequestError{http.StatusConflict, "model changed; reconnect to create a new upstream WebSocket", "subpool_websocket_model_changed"}
		}
	...
	if !s.pinAccount(account, nil, false, "") {
		return false, false, &RequestError{http.StatusServiceUnavailable, "WebSocket session is closing", "server_error"}
	}
	return true, !continuation, nil
}

func (s *responsesWSSession) initialAccount(request responsesWSRequest, route domain.KeyRoute) (domain.ProviderAccount, bool, *RequestError) {
	if request.PreviousResponseID != "" {
		account, err := s.hub.backend.RoutingStore().ResolveSessionAccount(s.ctx, route.Key.ID, responseevent.SessionHash(request.PreviousResponseID))
		if err != nil {
			if errors.Is(err, store.ErrNotFound) {
				return domain.ProviderAccount{}, true, &RequestError{http.StatusBadRequest, "previous response was not found", "previous_response_not_found"}
			}
			return domain.ProviderAccount{}, true, &RequestError{http.StatusServiceUnavailable, "session state is unavailable", "server_error"}
		}
		if !s.hub.backend.AccountHealthy(account) {
			return domain.ProviderAccount{}, true, &RequestError{http.StatusServiceUnavailable, "session account is unavailable", "subpool_session_account_unavailable"}
		}
		return account, true, nil
	}
	account := route.Account
	if route.Pool.Provider == domain.ProviderMixed && account.CredentialType == domain.CredentialAPIKey {
		if reassigned, err := s.hub.backend.RoutingStore().ReassignAPIKey(s.ctx, route.Key.ID, route.Pool.ID, nil); err == nil {
			account = reassigned
		}
	}
	if route.MembershipEnabled && s.hub.backend.AccountHealthy(account) {
		return account, false, nil
	}
	reassigned, err := s.hub.backend.RoutingStore().ReassignAPIKey(s.ctx, route.Key.ID, route.Pool.ID, []string{account.ID})
	if err != nil {
		return domain.ProviderAccount{}, false, &RequestError{http.StatusServiceUnavailable, "no eligible account", "subpool_no_eligible_account"}
	}
	return reassigned, false, nil
}

func (s *responsesWSSession) pinNativeAccount(route domain.KeyRoute, first domain.ProviderAccount, continuation bool, model string) (bool, *RequestError) {
	account := first
	...
	for attempt := 0; attempt < maxProviderAttempts; attempt++ {
		...
		next, err := s.hub.backend.RoutingStore().ReassignAPIKey(s.ctx, route.Key.ID, route.Pool.ID, attempted)
		if err != nil {
			return false, requestErr
		}
		account = next
	}
	return false, &RequestError{http.StatusServiceUnavailable, "no eligible account", "subpool_no_eligible_account"}
}

func (s *responsesWSSession) dialCodex(account domain.ProviderAccount, model string) (*websocket.Conn, domain.ProviderAccount, bool, *RequestError) {
	...
	if (status == http.StatusUnauthorized || status == http.StatusForbidden) && refreshableCredentials(account) {
		refreshed, refreshErr := s.hub.backend.RefreshAccount(s.ctx, account)
		if refreshErr != nil {
			s.hub.upstreamFailures.Add(1)
			if s.hub.backend.RecordRefreshFailure(s.ctx, account.ID, refreshErr) {
				return nil, account, true, &RequestError{http.StatusUnauthorized, "provider authentication failed", "provider_authentication_error"}
			}
			return nil, account, true, &RequestError{http.StatusServiceUnavailable, "provider credential refresh is unavailable", "provider_error"}
		}
	...
	case status == http.StatusUnauthorized || status == http.StatusForbidden:
		_ = s.hub.backend.RoutingStore().UpdateProviderStatus(s.ctx, account.ID, domain.AccountAuthFailed, nil)
		return nil, account, true, &RequestError{http.StatusUnauthorized, "provider authentication failed", "provider_authentication_error"}
	case status == http.StatusTooManyRequests:
		retryAt := s.hub.backend.RetryAfter(headers)
		_ = s.hub.backend.RoutingStore().UpdateProviderStatus(s.ctx, account.ID, domain.AccountCoolingDown, &retryAt)
		return nil, account, true, &RequestError{http.StatusTooManyRequests, "provider is rate limited", "subpool_rate_limited"}
	case status >= 500 || status == 0:
		s.hub.backend.RecordHealthFailure(s.ctx, account.ID, "websocket_connection_failed")
		return nil, account, true, &RequestError{http.StatusServiceUnavailable, "provider WebSocket is unavailable", "provider_error"}
	default:
		return nil, account, false, &RequestError{http.StatusBadGateway, "provider rejected WebSocket upgrade", "provider_error"}
	}
}

func (s *responsesWSSession) dialCodexOnce(account domain.ProviderAccount, model string) (*websocket.Conn, int, http.Header, error) {
	credentials, err := s.hub.backend.CodexCredentials(account)
	if err != nil {
...
func (s *responsesWSSession) validateContinuation(previous string) *RequestError {
	if previous == "" {
		return nil
	}
	s.mu.Lock()
	...
	account, err := s.hub.backend.RoutingStore().ResolveSessionAccount(s.ctx, s.keyID, responseevent.SessionHash(previous))
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			return &RequestError{http.StatusBadRequest, "previous response was not found", "previous_response_not_found"}
		}
		return &RequestError{http.StatusServiceUnavailable, "session state is unavailable", "server_error"}
	}
	if account.ID != accountID {
		return &RequestError{http.StatusConflict, "previous response belongs to another account", "subpool_session_account_mismatch"}
	}
	return nil
}

func (s *responsesWSSession) preparePayload(request responsesWSRequest) ([]byte, *RequestError) {
	s.mu.Lock()
	native := s.native
	account := s.account
	s.mu.Unlock()
	if native {
		return s.hub.backend.NormalizeCodexRequest(request.raw, account)
	}
	body, err := jsonobject.Parse(request.raw)
	if err != nil {
		return nil, &RequestError{http.StatusBadRequest, "invalid response.create message", "invalid_request_error"}
	}
	body.Delete("type")
...
func (s *responsesWSSession) reserveTurn(request responsesWSRequest, payload []byte, releaseBody func()) (*responsesWSTurn, *RequestError) {
	streamID := request.streamID()
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
		return nil, &RequestError{http.StatusServiceUnavailable, "WebSocket session is closing", "server_error"}
	}
	if s.nativeSwitching {
		s.mu.Unlock()
		return nil, &RequestError{http.StatusServiceUnavailable, "provider account failover is in progress", "provider_error"}
	}
	if s.pending >= responsesWSMaxTurnsPerConnection {
		s.mu.Unlock()
		return nil, &RequestError{http.StatusTooManyRequests, "connection turn capacity exceeded", "subpool_capacity_exceeded"}
	}
	if request.StreamID != nil {
		if _, known := s.named[streamID]; !known && len(s.named) >= responsesWSMaxNamedStreams {
			s.mu.Unlock()
			return nil, &RequestError{http.StatusBadRequest, "too many named streams", "invalid_stream_id"}
		}
	}
	accountID := s.account.ID
	s.mu.Unlock()
	if !s.hub.reserveAccountTurn(accountID) {
		return nil, &RequestError{http.StatusServiceUnavailable, "account turn capacity exceeded", "subpool_capacity_exceeded"}
	}
	turn := &responsesWSTurn{
		streamID: streamID, accountID: accountID, model: request.Model, payload: payload,
		raw: request.raw, continuation: request.PreviousResponseID != "", releaseBody: releaseBody,
	}
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
...
func (s *responsesWSSession) retryNativeLimitedTurn(turn *responsesWSTurn, failedUpstream *websocket.Conn, limitKind responsesWSLimitKind) bool {
	...
	if limitKind == responsesWSLimitQuota {
		_ = s.hub.backend.RoutingStore().SetProviderUsageAllowed(s.ctx, failedAccount.ID, false)
	} else {
		retryAt := s.hub.backend.Now().Add(time.Minute)
		_ = s.hub.backend.RoutingStore().UpdateProviderStatus(s.ctx, failedAccount.ID, domain.AccountCoolingDown, &retryAt)
	}
	attempted := []string{failedAccount.ID}
	next, err := s.hub.backend.RoutingStore().ReassignAPIKey(s.ctx, s.keyID, s.poolID, attempted)
	...
	for len(attempted) < maxProviderAttempts {
		...
		if requestErr == nil {
			payload, payloadErr := s.hub.backend.NormalizeCodexRequest(turn.raw, account)
			if payloadErr != nil {
				_ = conn.CloseNow()
				s.hub.backend.RecordHealthFailure(s.ctx, account.ID, "websocket_request_normalization_failed")
				...
				_ = conn.CloseNow()
				s.hub.upstreamFailures.Add(1)
				s.hub.backend.RecordHealthFailure(s.ctx, account.ID, "websocket_write_failed")
...
func (s *responsesWSSession) bridgeResponse(turn *responsesWSTurn, route domain.KeyRoute, account domain.ProviderAccount) (*http.Response, RetryReason, bool) {
	...
	return s.hub.backend.AttemptBridge(s.ctx, headers, route, account, turn.model, turn.payload)
}
...
func (s *responsesWSSession) finishTurn(turn *responsesWSTurn) {
	...
	releaseBody := turn.releaseBody
	turn.releaseBody = nil
	turn.raw = nil
	turn.payload = nil
	s.mu.Unlock()
	if releaseBody != nil {
	...
	s.hub.backend.CompleteTurn(s.keyID, s.poolID, accountID, turn.response, turn.model, turn.input, turn.output)
}
